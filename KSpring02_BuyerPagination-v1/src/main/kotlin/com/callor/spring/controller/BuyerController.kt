...
/**
 * 2021-10-26
 * pagination 을 적용 하기 위하여 controller method 를 변경한다
...
class BuyerController(val bService:BuyerService) {
     ...
     *      &sort=name,asc
     *
     * 만약 주문정보를 조회한다면 다음처럼 최근입력한 자료가 먼저 나타나도록 할때
     ...
     * id, name, value 값을 자동으로 채워넣는 기능을 만들수 있다
     *
     * thymeleaf template 을 사용할때는
     * form tag 에 model 에 담긴 object 를 지정해 주고
     * 각 input box 에서는 field 속성으로 해당 맴버변수(요소,속성)을 지정해 주면
    ...
    fun update(redirectAttributes: RedirectAttributes, buyer: Buyer, @PathVariable("userid") userid: String):String{
         ...
         * redirect 를 실행할때 model 담긴 변수를 queryString 으로 부착하여 전송을 한다
         * 이 기능이 boot 에서는 금지되고
         * 같은 기능을 구현하기 위하여
